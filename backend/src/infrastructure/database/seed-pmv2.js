...
function calidadGranoFromIndex(li) {
  if (li >= 3) return 'Excelente'
  if (li >= 1) return 'Buena'
  return 'Regular'
}

function calidadFinalFromPuntaje(puntaje) {
...
async function insertTrazabilidadForLote(loteId, fecha, parcela, li) {
  let orden = 1
...
async function insertCalidadForLote(loteId, codigo, fecha, pi, li) {
  const aroma = 7 + (li % 3)
  const attrs = [aroma, 7 + li, 8, 7 + (pi % 2), 8, 7 + li]
  const puntaje = Math.round((attrs.reduce((a, v) => a + v, 0) / 6) * 10)
  const calidad_final = calidadFinalFromPuntaje(puntaje)
  await execute(
    `INSERT INTO control_calidad (lote_id, aroma, sabor, cuerpo, acidez, dulzor, balance, puntaje_taza, calidad_final, estado, observaciones, fecha_evaluacion)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
    [loteId, ...attrs, puntaje, calidad_final, 'Evaluado', `Evaluación PMV2 — lote ${codigo} con perfil ${calidad_final.toLowerCase()}.`, fecha]
  )
}

async function insertPmv2Lote(productorId, parcela, altBase, pi, li, loteGlobal) {
  ...
  const ins = await execute(
    `INSERT INTO lotes (codigo_lote, productor_id, variedad_cafe, fecha_cosecha, cantidad_kg, estado, humedad, temperatura, altitud, tipo_secado, calidad_grano, tiempo_almacenamiento_dias, qr_codigo)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [codigo, productorId, variedad, fecha, cantidad, estado, humedad, temp, altitud, secado, calidadGranoFromIndex(li), 15 + li * 10, `CAFE-${loteGlobal}-PMV2`]
  )
  const loteId = ins.insertId

  await insertTrazabilidadForLote(loteId, fecha, parcela, li)
  await execute(`INSERT INTO inventario (lote_id, cantidad_disponible_kg, fecha_actualizacion) VALUES (?,?,CURDATE())`, [loteId, cantidad])
  await insertCalidadForLote(loteId, codigo, fecha, pi, li)

  return { id: loteId, codigo, humedad, temp, altitud, secado, variedad, fecha, parcela, li, pi }
}

async function seedPrediccionesPmv2(loteIds) {
  let predCount = 0
  for (const l of loteIds) {
    if (predCount >= 15 || (l.li % 2 === 0 && l.pi >= 3)) continue
    ...
    const ins = await execute(
      `INSERT INTO predicciones_ia (lote_id, humedad, temperatura, altitud, tipo_secado, variedad_cafe, calidad_predicha, confianza, porcentaje_riesgo, recomendacion, factores_influyentes, fecha_prediccion, modelo, origen, version_modelo)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'usuario',?)`,
      [l.id, l.humedad, l.temp, l.altitud, l.secado, l.variedad, result.calidad_predicha, result.confianza, result.porcentaje_riesgo, result.recomendacion, JSON.stringify(result.factores), l.fecha, result.modelo, result.version_modelo]
    )
...
export async function seedPMV2Data(force = false) {
  ...
  for (let pi = 0; pi < productorIds.length; pi++) {
    ...
    for (let li = 0; li < 5; li++) {
      loteIds.push(await insertPmv2Lote(productorId, parcela, altBase, pi, li, loteGlobal))
      loteGlobal++
    }
  }

  const predCount = await seedPrediccionesPmv2(loteIds)
