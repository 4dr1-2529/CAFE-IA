...
import { getLotes, getPredicciones, ejecutarPrediccionIA } from '../services/api.js'
...
export default function ModuloIA() {
  ...
  const prediccionesValidas = predicciones.filter(p => p?.lote_id && p?.codigo_lote && p?.productor)
  const lotesPredichos = new Set(prediccionesValidas.map(p => Number(p.lote_id)))
  const lotesPendientes = lotes.filter(l => !lotesPredichos.has(Number(l.id)))
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      // Backend toma las variables reales desde SQLite.
      ...
      <div className="bg-gradient-to-r from-cafeVerde-600 to-cafe-700 rounded-2xl p-6 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-cafe-100">Predicción bajo demanda, sin generación automática</p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className={`rounded-xl p-6 border-2 ${getColorPorCalidad(prediccion.calidad_predicha)}`}>
                <div className="text-center">
                  ...
                  <p className="text-2xl font-semibold mt-2">{prediccion.confianza}%</p>
