...
export default function Reportes() {
  ...
  const loadData = async () => {
    try {
      ...
      const totalPrediccionesReales = Number(pred?.resumen?.total ?? (Array.isArray(pred?.predicciones) ? pred.predicciones.length : 0))
      ...
      setStats({
        ...
        produccionMensual: prod?.porVariedad || [],
      ...
      <div className="bg-white rounded-xl shadow-sm border border-cafe-100 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button disabled={exporting} onClick={() => generarReporte(activeReport, 'pdf')} className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold py-2 px-4 rounded-lg hover:from-green-600 hover:to-green-700 transition-all disabled:opacity-50"><Download className="w-4 h-4" />PDF</button>
            <button disabled={exporting} onClick={() => generarReporte(activeReport, 'excel')} className="flex items-center gap-2 bg-cafe-700 text-white font-semibold py-2 px-4 rounded-lg hover:bg-cafe-800 transition-all disabled:opacity-50"><Download className="w-4 h-4" />Excel</button>
          </div>
        </div>
