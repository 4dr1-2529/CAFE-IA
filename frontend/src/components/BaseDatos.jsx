...
import { getProductores, getLotes, getProduccion, getTrazabilidad, getControlCalidad, getPredicciones } from '../services/api.js'
...
export default function BaseDatos() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const prediccionesValidas = Array.isArray(pred)
        ? pred.filter(p => p?.lote_id && p?.codigo_lote && p?.productor)
        : []
      setPredicciones(prediccionesValidas)
    } catch (err) {
      console.error('Error cargando datos:', err)
      setError('Error al cargar datos del backend. Verifica que el servidor esté activo en http://localhost:3001')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 text-blue-700 rounded-lg">
              ...
              <span className="text-sm font-medium">SQLite activo</span>
            </div>
            <button
              onClick={loadData}
