...
export function normalizeDashboardPayload(data, user) {
  ...
  if (isAdmin) {
    ...
    cards.evaluacionesCalidad = Number(cards.evaluacionesCalidad ?? data.kpis?.evaluacionesTotal ?? 0)
    cards.prediccionesIA = Number(cards.prediccionesIA ?? data.kpis?.prediccionesTotal ?? 0)
    cards.accionesAuditoria = Number(cards.accionesAuditoria ?? 0)
  } else {
    cards.misProductores = Number(cards.misProductores ?? 0)
    cards.misLotes = Number(cards.misLotes ?? data.kpis?.totalLotes ?? 0)
    cards.misLotesConTrazabilidad = Number(cards.misLotesConTrazabilidad ?? cards.lotesConTrazabilidad ?? 0)
    ...
    cards.misEvaluacionesCalidad = Number(cards.misEvaluacionesCalidad ?? cards.evaluacionesCalidad ?? data.kpis?.evaluacionesTotal ?? 0)
